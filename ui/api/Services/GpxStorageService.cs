...
public class GpxStorageService
{
    ...
    public async Task<LocalFileLease> ExtractOriginalToTempAsync(string key, CancellationToken ct = default)
    {
        ...
        //
        // entry.FullName belongs to the archive, not to us. We write these archives
        // ourselves today, but they round-trip through object storage that other
        ...
        if (!extractedPath.StartsWith(extractRoot, StringComparison.Ordinal))
            throw new InvalidOperationException(
                $"Archive entry escapes the extraction directory: {archiveKey}");
