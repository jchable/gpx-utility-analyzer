    ...
    <div className="space-y-6">
      ...
      {activities && activities.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-slate-500 text-sm mt-1">{t('emptyStateHint')}</p>
        </div>
      )}
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <p className="text-xs text-slate-500 mb-0.5">{t('distance')}</p>
                    <p className="text-sm font-semibold text-white">{activity.distanceKm.toFixed(1)} {tc('unit.km')}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 mb-0.5">{t('elevationGain')}</p>
                    <p className="text-sm font-semibold text-white">{Math.round(activity.elevationGainM)} {tc('unit.m')}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 mb-0.5">{t('time')}</p>
                    <p className="text-sm font-semibold text-white">{formatDuration(activity.movingTimeSeconds)}</p>
                  </div>
                </div>
                ...
                <div className="mt-4 flex items-center gap-2">
                  ...
                  />
                  <span className="text-xs text-slate-500">{tc(`status.${activity.status}`)}</span>
